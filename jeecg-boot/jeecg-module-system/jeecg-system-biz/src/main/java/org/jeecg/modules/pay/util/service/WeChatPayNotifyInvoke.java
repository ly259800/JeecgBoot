...
import org.jeecg.modules.pay.constants.EventTypeEnum;
...
import org.jeecg.modules.pay.entity.CallbackParams;
...
public class WeChatPayNotifyInvoke {
    ...
    public void execNotify(ResponseSignVerifyParams params) {
        ...
        // 5.根据openid获取支付人信息
        SysUser user = sysUserService.getUserByOpenId(consumeData.getPayer().getOpenid());
        if(Objects.nonNull(user)){
            riderUserOrder.setPayId(user.getId());
            riderUserOrder.setPayer(user.getRealname());
        }
        // 6.更新支付订单、租户订单及租户信息
